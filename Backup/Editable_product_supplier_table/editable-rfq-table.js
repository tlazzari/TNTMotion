...
>
  ...
  <td>${row.client_name || ''}</td> <!-- Updated to display client_name -->
  <td>${row.rfq_supplier_id || ''}</td>
  <td>${row.rfq_product_id || ''}</td>
  <td>${row.supplier_name || ''}</td>
  <td>${row.product_name || ''}</td>
  <td><input type="text" class="quantity-input" value="${row.quantity || ''}"></td>
  <td><input type="number" step="0.01" class="price-input" value="${row.price||''}"></td>
  ...
  <td>
    <input
      type="date"
      class="promised-delivery-date-input"
      value="${ dateValue }"
    >
  </td>
  ...
  $("#editable-rfq-table").on("blur change",
    ...
    ".promised-delivery-date-input, .status-select",
    function(){
