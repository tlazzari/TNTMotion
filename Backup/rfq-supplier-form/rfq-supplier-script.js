jQuery(function($){
  ...
  $rfqTable.on('click','.select-rfq-product',function(){
    selectedRFQProduct = {
      rfq_product_id: $(this).data('id'),
      rfq_client_id:  $(this).data('client-id'),
      product_family: $(this).data('family'),
  ...
  function fetchSuppliers() {
    $.post(rfqSupplierAjax.ajax_url, {
      ...
      if (!res.success) {
        $supplierTable.find('tbody')
          .html('<tr><td colspan="6">Error: '+res.data.message+'</td></tr>');
      } else {
