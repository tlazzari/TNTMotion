...
function unique_order_details_shortcode() {
    ...
    $query = "
        SELECT 
            ...
            so.notes
        FROM supplier_order so
        JOIN rfq_client rc ON so.rfq_client_id = rc.rfq_client_id
        JOIN client c ON rc.client_id = c.client_id
        JOIN rfq_supplier rs ON so.rfq_supplier_id = rs.rfq_supplier_id
    ...
    if ($result && $row = $result->fetch_assoc()) {
        ...
        foreach ($row as $key => $value) {
            $output .= "<li class='list-group-item'><strong>" . ucfirst(str_replace('_', ' ', $key)) . ":</strong> " . esc_html($value) . "</li>";
        }
        $output .= '</ul>';
    } else {
