...
$query = "
  SELECT
    ...
    lastq.price,
    lastq.currency_id,
    lastq.promised_delivery_date,
    currency.currency_description,
    GROUP_CONCAT(
      DISTINCT rfq_supplier.notes
      ORDER BY rfq_supplier.notes ASC
      SEPARATOR ', '
    ) AS supplier_notes
  FROM rfq_supplier

  -- Join the product row
  LEFT JOIN rfq_product
    ON rfq_supplier.rfq_product_id = rfq_product.rfq_product_id

  -- Pick exactly one (latest) quotation per supplier line
  LEFT JOIN (
    SELECT
      q1.rfq_supplier_id,
      q1.quotation_id,
      q1.price,
      q1.currency_id,
      q1.promised_delivery_date
    FROM (
      SELECT
        rfq_supplier_id,
        MAX(quotation_id) AS max_qid
      FROM quotation
      GROUP BY rfq_supplier_id
    ) AS m
    JOIN quotation AS q1
      ON q1.rfq_supplier_id = m.rfq_supplier_id
     AND q1.quotation_id    = m.max_qid
  ) AS lastq
    ON lastq.rfq_supplier_id = rfq_supplier.rfq_supplier_id

  -- Currency lookup
  LEFT JOIN currency
    ON lastq.currency_id = currency.currency_id
  ...
  -- no-op WHERE so we can append filters easily
  WHERE 1=1
...
";
    ...
    if ( ! $res ) {
        // log the failing SQL and MySQL’s error:
        error_log( "RFQ fetch failed:\nSQL: $query\nMySQL: " . $conn->error );
        wp_send_json_error([
...
function update_rfq_table_data() {
    ...
    try {
     ...
     //
    // 8) Update or Insert into quotation
    //
    // See if the client sent us an existing quotation_id:
    ...
    if ( $quotation_id > 0 ) {
        ...
        // no quotation_id => insert a new one
        $sql = "
