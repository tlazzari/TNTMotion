...
$('#tax-rate-input').on('change blur', function(){
  ...
  $('body')
    ...
    .on('click','.add-supplier-bank', function(){
    const id = $(this).data('doc-id');
    window.location.href = otpData.base_url
      + '?view=add_supplier_bank&order_document_id=' + id;
  });
...
$('body').on('click', '#otp-unassign-lines', function(){
  const docId = $(this).data('doc-id'),
        ids   = $('.unassign-line-checkbox:checked').map(function(){
                  return $(this).data('order-id');
                }).get();
  if (!ids.length) {
    return alert('Please select at least one line to unassign.');
  }
  if (!confirm('Really unassign '+ids.length+' line(s)?')) return;

  $.post(otpData.ajax_url, {
    action:              'otp_unassign_order_lines',
    nonce:               otpData.nonce,
    order_document_id:   docId,
    selected_order_ids:  ids
  }, function(resp){
    if (resp.success) {
      // fade them out
      ids.forEach(id => {
        $('.unassign-line-checkbox[data-order-id="'+id+'"]').closest('tr')
          .fadeOut(200, function(){ $(this).remove(); });
      });
    } else {
      alert('Unassign failed: ' + resp.data);
    }
  }, 'json');
});
