...
document.addEventListener('DOMContentLoaded', function() {
  // 1) Strip any rfq_error or product_error from the URL
  const params = new URLSearchParams(window.location.search);
  if (params.has('rfq_error') || params.has('product_error')) {
    params.delete('rfq_error');
  ...
  // 2) Remove any existing error banners in the DOM
  [
    '.rfq-message.rfq-error',
    'p.woocommerce-error',
    '.woocommerce-error',
    '.notice.notice-error',
    '.error'
  ...
  // 3) Remove that specific “First Create a RFQ…” paragraph
  document.querySelectorAll('p').forEach(p => {
    if (p.textContent.trim() ===
        'First Create a RFQ from a Client and Then add different products to it.') {
      p.remove();
    }
  });

  // — now your existing initialization code —

  const rfqForm      = document.getElementById('rfq_form');
  const detailsTable = document.getElementById('rfq-details-table');
  if (!rfqForm && !detailsTable) return;

  console.log("Initializing RFQ…");

  if ( rfqForm ) {
    // Hide the entire RFQ form until the user clicks “Add New RFQ”
...
function populateExistingRfqsTable(rfqs){
  ...
  existingRfqsTableContainer.style.display = 'block';
     ...
     */
function initSingleRfqTable() {
  // Define which statuses to hide
  var hiddenStatuses = [7, 8, 19];
  ...
  tableInstance = jQuery('#single_rfq_table').DataTable({
    ...
    searching:    false,
    lengthChange: false,
    autoWidth:    false,            // Turn off auto-width so our widths stick
    scrollX:      true,             // Allow horizontal scrolling when needed
    order: [
      [6, 'asc'],   // first sort by Valid Quotes
      [1, 'asc']    // then by RFQ Client ID
    ],

    // Pre-filter rows in the AJAX dataSrc so bad statuses never get in
    ajax: {
      url:  rfq_ajax_object.ajax_url,
      type: 'POST',
      data: { action: 'srfq_fetch_lines', nonce: rfq_ajax_object.nonce },
      dataSrc: function(json) {
        if (!json.success) {
          return [];
        }
        // filter out undesired statuses here
        return json.data.rows.filter(function(row) {
          return hiddenStatuses.indexOf(row.status_rfq_product) === -1;
        });
      }
    },

    // Lock in exact widths for the key columns
    columnDefs: [
      { width: '60px', targets: 4 },   // Quantity
      { width: '80px', targets: 7 },   // Target Price Bf VAT
      { width: '80px', targets: 9 }    // Price to Client Bf VAT
    ],
    ...
    columns: [
      ...
      },
      {
        ...
        title: 'Suppliers',
        render: function(v, t) {
          return t === 'display' ? '<span>' + v + '</span>' : v;
        }
      },
      {
      ...
      },
      {
        ...
        render: function(val, type, row) {
          ...
          var opts = rfq_ajax_object.currencies.map(function(c) {
            return '<option value="' + c.id + '"' +
                   (c.name === val ? ' selected' : '') +
                   '>' + c.name + '</option>';
          }).join('');
          return '<select class="currency-select" data-rfq-product-id="' +
                 row.rfq_product_id + '">' + opts + '</select>';
        }
      },
      {
        data: 'price_to_client',
        title: 'Price to Client Bf VAT',
        render: function(v, t) {
          return t === 'display'
    ...
    createdRow: function(row, data) {
      var zeroQuotes      = (data.quotations_count === 0);
      var zeroClientPrice = (parseFloat(data.price_to_client) === 0);
      var hasClientPrice  = (parseFloat(data.price_to_client) > 0);
      var badA = [7,8,19,16];
      var badB = [11,7,8,19,16];
      var sid  = parseInt(data.status_rfq_product, 10);

      if (zeroQuotes && zeroClientPrice && badA.indexOf(sid) === -1) {
        jQuery(row).addClass('highlight-red');
      } else if (zeroQuotes && hasClientPrice && badB.indexOf(sid) === -1) {
        jQuery(row).addClass('highlight-red');
      }
    }
  });

  // Make sure the browser lays out the fixed widths properly
...
jQuery('#download_xlsx_btn').after(
  ...
  '<button id="analyze_quote_btn"   class="rfq-button" style="margin-left:8px;">Analyze Quote</button>' +
  '<button id="assign_supplier_btn" class="rfq-button" style="margin-left:8px;">Assign New Supplier</button>' +
  '<button id="add_new_rfq_btn"     class="rfq-button" style="margin-left:8px;">Add New RFQ</button>'
);


// Helpers + Handlers must come *after* you injected the buttons
function getSelectedRfqProductId() {
  const $checked = jQuery('#single_rfq_table tbody input.row-select:checked');
  if ($checked.length !== 1) {
...
jQuery('#analyze_quote_btn').on('click', function() {
  const id = getSelectedRfqProductId();
  if (id) {
    window.location.href = `https://tntbearings.com/clients-and-rfq/?rfq_product_id=${id}`;
  }
});

jQuery('#assign_supplier_btn').on('click', function() {
  const id = getSelectedRfqProductId();
  if (id) {
    window.location.href = `https://tntbearings.com/suppliers-and-rfq/?rfq_product_id=${id}`;
  }
});
    ...
    jQuery('#view_all_btn').on('click', function() {
      filterActive = false;
      tableInstance.draw();                // <-- re-draw applies your ext.search
      jQuery(this).prop('disabled', true).text('All Rows Shown');
    });

    jQuery('#add_new_rfq_btn').on('click', function() {
  // reveal the FORM container
  jQuery('.rfq-form-container').slideDown({
    start: () => {
      // *only once*, right when we start to show it:
      initializeForm();
    }
  });
  window.scrollTo({
    top: document.querySelector('.rfq-form-container').offsetTop - 20,
    behavior: 'smooth'
  });
  ...
  // ————— Only initialize the RFQ _form_ —
    ...
    console.log('Attaching saveLive handlers to #rfq-details-table');

    function saveLive(e){
      const row = e.target.closest('tr');
      const fd  = new FormData();
      fd.append('action',            'srfq_save_live_prices');
      fd.append('nonce',             rfq_ajax_object.nonce);
      fd.append('rfq_product_id',    e.target.dataset.rfqProductId);
      fd.append('rfq_supplier_id',   e.target.dataset.rfqSupplierId); // ← you must render this in your HTML!
      fd.append('original_price',    parseFloat(row.querySelector('.js-edit-original').value)     || 0);
      fd.append('price_to_client',   parseFloat(row.querySelector('.js-edit-client-price').value) || 0);

      fetch(rfq_ajax_object.ajax_url, { method:'POST', body:fd })
        .then(r=>r.json())
        .then(resp=>{
          if (!resp.success) {
            alert('Could not save price: '+ (resp.data?.message||'Unknown'));
          }
        });
    }


    // delegate change events on both inputs
    const detailsTable = document.getElementById('rfq-details-table');
if ( detailsTable ) {
  detailsTable.addEventListener('change', function(e){
    if ( e.target.matches('input.js-edit-original, input.js-edit-client-price') ) {
      saveLive(e);
    }
  });
}
  }  // ← end if(document.getElementById('rfq-details-table'))
